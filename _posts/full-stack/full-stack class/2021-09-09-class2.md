...
![image-20210924104202554](../../../assets/images/image-20210924104202554.png)
...
`str.getBytes("인코딩방식")` str 문자열을 인코딩방식으로 인코딩하여 바이트배열에 담는다.
...
- `nextInt(bound)` 0 <= 수 < bound 의 랜덤숫자를 반환한다.
...
try (Scanner scanner = new Scanner(System.in);){
    ...
    int year = Integer.parseInt(source.split("/")[0]);
    int month = Integer.parseInt(source.split("/")[1]) - 1;
    int date = Integer.parseInt(source.split("/")[2]);
...
4. LocalDateTime + 시간대 = ZoneDateTime
