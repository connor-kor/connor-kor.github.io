...
```java
Arrays.sort(files, Comparator.comparingLong(i -> ((File) i).length()).reversed());
```
...
for (File f : new File(path).listFiles()) {
    int n = Integer.parseInt(FileUtil.getBaseName(f.getName()));
    f.renameTo(new File(path,String.format("%04d", n) + ".png"));
}
```
...
for (File f : new File(path).listFiles()) {
    int n = Integer.parseInt(FileUtil.getBaseName(f.getName()));
    f.renameTo(new File(path, n + ".png"));
}
```
...
comeutil 손보기 및 split 과 count++ 로직 속도비교와 Buffer StringReader 속도비교
