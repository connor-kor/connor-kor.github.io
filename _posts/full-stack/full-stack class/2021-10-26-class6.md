...
```sql
SELECT rownum, pay
FROM (SELECT coalesce(sal + comm, sal) AS pay
    FROM emp
    ORDER BY pay DESC)
WHERE rownum <= 3;
```
...
SELECT ename, coalesce(sal, sal + comm) AS pay
    , RANK() OVER( ORDER BY coalesce(sal, sal + comm) DESC) AS 순번
FROM emp)
SELECT *
FROM temp
...
WITH temp AS (
    SELECT ename, coalesce(sal, sal + comm) AS pay
        ...
        , RANK() OVER( ORDER BY coalesce(sal, sal + comm) DESC) AS rank
...
SELECT deptno, ename, sal AS pay
FROM emp o
WHERE sal = (SELECT max(sal) max_pay
    FROM emp
